...
perl timeout.pl -c -x 1 -t 8 ./pipexec -k -- [A /bin/bash returnRedirect java -cp "Problems/${problemID}" $(basename "Problems/${problemID}/$inputFile" .class) $1 ] [B /bin/bash returnRedirect java "-Xmx64m" "-Xms8m" -cp "Users/${userID}/${problemID}/${submitID}" $(basename "Users/${userID}/${problemID}/${submitID}/${i}" .java) ] [COPY1 /usr/bin/tee -a $log ] [COPY2 /usr/bin/tee -a $log ] [COPY3 /usr/bin/tee $teacherReturn ] [COPY4 /usr/bin/tee $studentReturn ] [PRE1 /bin/bash predate.sh "Teacher: " ] [PRE2 /bin/bash predate.sh "Student: " ] [ERR1 /usr/bin/tee $graderErr ] [ERR2 /usr/bin/tee $studentErr ] "{A:3>COPY3:0}" "{B:3>COPY4:0}" "{A:1>PRE1:0}" "{PRE1:1>B:0}" "{PRE1:100>COPY1:0}" "{B:1>PRE2:0}" "{PRE2:1>A:0}" "{PRE2:100>COPY2:0}" "{A:2>ERR1:0}" "{B:2>ERR2:0}" > /dev/null 2> $timeoutOut
execrtrn=$?
