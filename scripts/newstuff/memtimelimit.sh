...
sub print_uinfo
{
	...
	if ($reason eq 'FINISHED'){
		printf STDERR "Program Completed Succsefully\n"
	} elsif ($reason eq 'TIMEOUT') {
		printf STDERR "Program took too long\n"
	} elsif ($reason eq 'MEM') {
		printf STDERR "Program used too much memory\n"
	} elsif ($reason eq 'SIGNAL') {
		printf STDERR "Program was ended by the server\n"
	} else {
		printf STDERR "UNKNOWN RESUALT: ${id_str}%s\n", $reason
	}
	printf STDERR "CPU Time: %.2f seconds\nEnd Memory %d kilobytes\nPeak Memory: %d kilobytes\n", $timeinfo->{total}, $meminfo, $maxmem if ($reason ne 'FINISHED') || $info_on_success;
